= Net Install ISO
:description: Use the Net Install ISO to install a minimal operating system that pulls container images from the internet after installation, which is useful when virtual
:revdate: 2026-01-08
:page-revdate: {revdate}

[WARNING]
====
A critical vulnerability in the {harvester-product-name} interactive installer allows attackers to gain remote shell (SSH) access to the host. If your environment is exposed to untrusted locations, you must either upgrade to v1.7 and use the corresponding interactive installer, or use the xref:installation-setup/methods/pxe-boot-install.adoc[PXE boot installation mechanism] when creating or expanding clusters. For more information, see the https://github.com/harvester/harvester/security/advisories/GHSA-6g8q-hp2j-gvwv[security advisory for CVE-2025-62877].
====
