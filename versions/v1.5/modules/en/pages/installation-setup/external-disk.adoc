...
:description: Install SUSE Virtualization on external iSCSI devices by configuring firmware settings, enabling UEFI boot, setting up the iSCSI initiator and target
:revdate: 2025-12-05
...
[IMPORTANT]
====
The `multiPathConfig` structure changed starting in v1.7.0. While this change does not affect upgrades, you must review the https://documentation.suse.com/cloudnative/virtualization/v1.7/en/installation-setup/config/configuration-file.html#_os_externalstorageconfig[`os.externalStorageConfig`] documentation before performing a fresh installation of v1.7.0 or later versions.
====
