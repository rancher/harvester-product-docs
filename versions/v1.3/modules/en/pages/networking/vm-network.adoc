...
Harvester also introduced storage networking to separate the storage traffic from other cluster-wide workloads. Please refer to xref:networking/storage-network.adoc[the storage network document] for more details.
...
The usage of untagged network is similar to xref:networking/vm-network.adoc#_how_to_use_vlan_network[the VLAN network].
