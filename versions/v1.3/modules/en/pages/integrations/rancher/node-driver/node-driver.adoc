...
Click to learn xref:integrations/rancher/node-driver/rke1-cluster.adoc[how to create RKE1 Kubernetes Clusters].

== RKE2 Kubernetes cluster

Click to learn xref:integrations/rancher/node-driver/rke2-cluster.adoc[how to create RKE2 Kubernetes Clusters].

== K3s Kubernetes cluster

Click to learn xref:integrations/rancher/node-driver/k3s-cluster.adoc[how to create k3s Kubernetes Clusters].
...
. Create a downstream RKE2 cluster using the Harvester node driver with Harvester cloud provider enabled. We recommend adding the xref:integrations/rancher/node-driver/rke2-cluster.adoc#add-node-affinity[node affinity rules], which prevents nodes from drifting to other zones after VM rebuilding.
+
...
For Harvester cloud provider < v0.2.0, label synchronization will only occur during the initialization of guest nodes. To prevent nodes from drifting to different regions or zones, we recommend adding xref:integrations/rancher/node-driver/rke2-cluster.adoc#_add_node_affinity[node affinity rules] during cluster provisioning. This will allow you to schedule VMs in the same zone even after rebuilding.
====
