...
Logging operations are backed by the https://kube-logging.dev/docs/#overview[Logging Operator] and controlled using Fluentd resources, particularly https://kube-logging.dev/docs/configuration/flow/[Flow and ClusterFlow] and https://kube-logging.dev/docs/configuration/output/[Outputs and ClusterOutput]. You can route and filter logs by applying these CRDs to the Harvester cluster.
