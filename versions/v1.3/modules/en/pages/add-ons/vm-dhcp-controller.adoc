...
. On the *Dashboard* screen of the Harvester UI, xref:../../networking/harvester-network.adoc#_create_a_vm_network[create a VM Network].
+
...
. xref:../../vm/create-vm.adoc#_how_to_create_a_vm[Create a VM] that is connected to the VM Network you previously created.
+
...
. Check the xref:../../vm/access-to-the-vm.adoc#_access_with_the_harvester_ui[VM's serial console] and verify that the IP address is correctly configured on the network interface (via DHCP).
+
