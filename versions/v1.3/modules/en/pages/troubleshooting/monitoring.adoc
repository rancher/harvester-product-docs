...
Access the embedded Longhorn WebUI according to xref:../troubleshooting/harvester.adoc#_access_embedded_rancher_and_longhorn_dashboards[this document].
...
You may encounter this when you install the Harvester v1.3.0 or higher version cluster with the minimal 250 GB disk per xref:../install/requirements.adoc#_hardware_requirements[hardware requirements].
...
When the issue exists and you xref:../upgrade/automatic.adoc#_start_an_upgrade[start an upgrade], Harvester may return the following error message: `admission webhook "validator.harvesterhci.io" denied the request: managed chart rancher-monitoring-crd is not ready, please wait for it to be ready`.
