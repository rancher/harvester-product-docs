...
An *Upgrade* button appears on the *Dashboard* screen whenever a new Harvester version that you can upgrade to becomes available. For more information, see xref:./automatic.adoc#_start_an_upgrade[Start an upgrade].

For air-gapped environments, see xref:./automatic.adoc#_prepare_an_air_gapped_upgrade[Prepare an air-gapped upgrade].
