...
:description: Troubleshoot virtual machine issues by using the kubectl delete pod command to delete the pod and make the Start button visible on SUSE
:revdate: 2025-12-05
...
[#_vm_objects]
=== Objects related to virtual machines
...
. <<#_vm_objects,Stop>> the affected VMs from the {harvester-product-name} UI.
+
...
. <<#_vm_objects,Start>> the affected VMs from harvester UI.
+
...
The workaround is to disable IPv6 in the xref:troubleshooting/operating-system.adoc#_how_to_permanently_edit_kernel_parameters[kernel parameters] of {harvester-product-name}.
