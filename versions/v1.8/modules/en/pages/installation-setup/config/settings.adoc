...
*Definition*: Setting that allows you to customize the `{string}` section of the support bundle file name.

When generating a support bundle, {harvester-product-name} creates a ZIP file with a name that follows the format `supportbundle_{string}_{timestamp}.zip`. If you do not specify a value, {harvester-product-name} uses a UUID instead.

|===
| `{string}` value | File name format | Example

| User-specified
| `supportbundle_{name}_{timestamp}.zip`
| `supportbundle_my-cluster_2025-09-17T03-39-17Z.zip`

| Empty
| `supportbundle_{uuid}_{timestamp}.zip`
| `supportbundle_c2eb6186-525f-411e-91ee-e88ed090d66d_2025-09-17T03-39-17Z.zip`
|===

[NOTE]
====
You can set the `{string}` value using environment variables. This can help identify support bundles from specific clusters or environments.
====
