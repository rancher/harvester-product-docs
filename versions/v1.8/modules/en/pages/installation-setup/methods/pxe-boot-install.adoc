...
:description: Install SUSE Virtualization automatically using iPXE for PXE boot, configure HTTP and DHCP servers, prepare boot files, and set up scripts for the cluster
:revdate: 2026-05-05
...
[IMPORTANT]
====
Support for legacy BIOS booting was deprecated in v1.7.0 and will be removed in a later release. Existing {harvester-product-name} clusters that use this boot mode will continue to function, but upgrading to later versions may require re-installation in UEFI mode. Starting with v1.8.0, PXE boot installation fails on legacy BIOS systems to ensure that UEFI is used for all new deployments.
====
