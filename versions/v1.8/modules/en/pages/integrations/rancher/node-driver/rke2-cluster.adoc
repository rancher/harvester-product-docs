...
:description: Create an RKE2 Kubernetes cluster using the Harvester Node Driver by setting up cloud credentials and configuring node and workload affinity rules,
:revdate: 2026-06-12
...
== Guest cluster provisioning failure after re-registering {harvester-product-name} cluster

Guest cluster provisioning may fail with an error similar to the following:
...
This can occur if you remove a {harvester-product-name} cluster from {rancher-short-name} and later re-register the same cluster, leaving behind stale cloud credentials.

For more information, see xref:integrations/rancher/cloud-provider.adoc#_stale_cloud_credentials_after_cluster_registration[Stale cloud credentials after cluster registration].
