...
These roles are defined using https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/manage-role-based-access-control-rbac/custom-roles[role templates] derived from {rancher-short-name}'s built-in roles. They complement the existing {harvester-product-name} and {rancher-short-name} integration model, which seamlessly maps {rancher-short-name} user permissions to {harvester-product-name} resources.

== Installation

The RBAC integration is available as a Helm chart in {rancher-short-name} 2.14.1 and later, accessible directly from the https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/helm-charts-in-rancher#access-charts[charts catalog].
...
The {harvester-product-name}-{rancher-short-name} RBAC provides the following project roles:
...
| *Manage Virtualization Resources*
| Project
| Allowed (only within projects where they hold membership)
|===
