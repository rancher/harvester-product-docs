...
:description: Learn networking best practices for replacing Ethernet NICs in a SUSE Virtualization cluster to ensure stable and efficient network operations
:revdate: 2025-12-26
...
* Wait for everything to become ready, and then repeat the steps in the <<#_pre_replacement_checks,Pre-check>> section.
...
. Replace the NICs.
. Restart the node.
. <<#_collecting_information,Collect information>> about the current network configuration and status.
