...
Clusters running v1.5.0 and v1.5.1 can upgrade to v1.6.x directly because {harvester-product-name} allows a maximum of one minor version upgrade for underlying components. v1.5.0, v1.5.1, and v1.5.2 use the same minor version of {rke2-short-name} (v1.32), while v1.6.0 uses the next minor version (v1.33). For more information, see xref:upgrades/upgrades.adoc#_upgrade_paths)[Upgrade paths].
