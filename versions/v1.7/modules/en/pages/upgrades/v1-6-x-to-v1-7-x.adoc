...
[CAUTION]
====
v1.7.x uses NetworkManager instead of wicked, which was used in earlier versions of {harvester-product-name}. If you modified the management interface configuration after the initial installation, you must perform additional manual steps to avoid issues during the upgrade. For more information, see <<#_migration_from_wicked_to_networkmanager>>.
...
[#_migration_from_wicked_to_networkmanager]
