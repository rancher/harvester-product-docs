...
You have the option to pause automatic upgrades on specific nodes, which is useful when manual maintenance or verification tasks must be performed. Following the completion of these tasks, you must explicitly instruct {harvester-product-name} to resume the upgrade on the target nodes.
