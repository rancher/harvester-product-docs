...
If the upgrade fails at this point, check the status of the repository virtual machine and its corresponding pod before <<#_restarting_the_upgrade,restarting the upgrade>>. You can check the status using the command `kubectl get vm -n harvester-system`.
...
If the upgrade fails at this point, check the job logs in the `cattle-system` namespace before <<#_restarting_the_upgrade,restarting the upgrade>>. You can check the logs using the command `kubectl get jobs -n cattle-system | grep prepare`.
...
[CAUTION]
====
If the upgrade fails at this point, you must generate a xref:troubleshooting/operating-system.adoc#_generate_a_support_bundle[support bundle] before <<#_restarting_the_upgrade,restarting the upgrade>>. The support bundle contains logs and resource manifests that can help identify the cause of the failure.
====
