...
[#_non_migratable_virtual_machines]
...
[#_live_migratable_virtual_machines]
...
When a migration is triggered from the <<#_starting_a_migration,Migrate>> menu item, the `VirtualMachineInstanceMigration` object's name is prefixed with the virtual 
...
* The virtual machine is <<#_non_migratable_virtual_machines,non-migratable>>.
