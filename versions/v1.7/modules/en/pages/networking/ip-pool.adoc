...
[#_creating_an_ip_pool_rancher_ui]
...
* Create an IP pool <<#_creating_an_ip_pool,using the {harvester-product-name} UI>>, which leaves the scope selector's *Project* and *Guest Kubernetes Cluster* fields blank.

* If you must <<#_creating_an_ip_pool_rancher_ui,use the {rancher-short-name} UI>>, set the *Project* and *Guest Kubernetes Cluster* fields to either *All* or *None*.

=== IP pool for guest cluster type load balancer

* Create an IP pool <<#_creating_an_ip_pool_rancher_ui,using the {rancher-short-name} UI>>, which allows you to specify the target project, namespace, and guest Kubernetes cluster for better resource management and isolation.
