...
* You have the `kubeconfig` file for your bare-metal {harvester-product-name} cluster. You can find the `kubeconfig` file from one of the {harvester-product-name} management nodes in the `/etc/rancher/rke2/rke2.yaml` path.
+
[,shell]
----
...
The Harvester CSI Driver provides the interface for defining the default StorageClass. If the default StorageClass in unspecified, the Harvester CSI Driver uses the default StorageClass of the host {harvester-product-name} cluster.

You can use the parameter `host-storage-class` to customize the default StorageClass.

. Create a StorageClass for the host {harvester-product-name} cluster.
+
Example:
...
Beginning with Harvester CSI Driver v0.1.15, it's possible to create a PersistentVolumeClaim (PVC) using a different {harvester-product-name} StorageClass on the guest Kubernetes Cluster.
...
. For administrators, you can create a desired xref:storage/storageclass.adoc[StorageClass] (e.g., named *replica-2*) in your bare-metal {harvester-product-name} cluster.
+
 ...
 ** For guest Kubernetes owners, you may request that the {harvester-product-name} cluster administrator create a new StorageClass.
...
. You can now create a PVC based on this new *StorageClass*, which utilizes the *Host StorageClass* to provision volumes on the bare-metal {harvester-product-name} cluster.
...
* You have created an RWX StorageClass on the host {harvester-product-name} cluster.
+
...
+
On the *StorageClass: Create* screen, add a *Host Storage Class* parameter and specify the RWX StorageClass that you created on the host {harvester-product-name} cluster.
+
