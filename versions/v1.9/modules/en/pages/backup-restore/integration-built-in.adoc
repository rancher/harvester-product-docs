...
Longhorn V2 Data Engine known issue::
Although backups and snapshots for Longhorn V2 Data Engine volumes are supported from v1.7.0, deleting the latest backup of a virtual machine, or enabling the {longhorn-product-name} _Auto Cleanup Snapshot When Delete Backup_ setting, blocks all subsequent operations (snapshots, backups, and live migration) on the related volumes. This is a https://github.com/longhorn/longhorn/issues/9064[known issue] with no viable workaround; recovery requires deleting the affected volume. See the caution in <<#_vm_backup_restore>>.

Minimum scheduling interval::
Scheduled backups and snapshots must have an interval of *at least one hour*, and schedules of the same granularity must be offset by *at least 10 minutes*. This bounds the shortest xref:backup-restore/definitions.adoc[RPO] achievable with the built-in scheduler. Workloads that require a shorter RPO need a different approach.
