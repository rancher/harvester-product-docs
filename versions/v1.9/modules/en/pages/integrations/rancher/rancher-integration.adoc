...
:description: Integrate SUSE Rancher Prime with SUSE Virtualization to centrally manage virtual machines and containers using Rancher's authentication and RBAC features
:revdate: 2026-05-15
...
For production setups, you must install the {rancher-short-name} server on a dedicated, high-availability Kubernetes cluster (with three or more nodes) deployed on infrastructure external to the {harvester-product-name} cluster. Ensure that the cluster nodes meet the https://documentation.suse.com/cloudnative/rancher-manager/v2.11/en/installation-and-upgrade/requirements/requirements.html[software, hardware, and networking requirements] outlined in the {rancher-short-name} documentation. For more information, see https://documentation.suse.com/cloudnative/rancher-manager/v2.11/en/installation-and-upgrade/quick-start/deploy-rancher/deploy-rancher.html[Deploying {rancher-product-name} Server].

If you want to test the integration features, you can create a virtual machine in {harvester-product-name} and then install the {rancher-short-name} server using Helm. For more information, see the https://documentation.suse.com/cloudnative/rancher-manager/v2.11/en/installation-and-upgrade/quick-start/deploy-rancher/helm-cli.html[Helm CLI Quick Start].
