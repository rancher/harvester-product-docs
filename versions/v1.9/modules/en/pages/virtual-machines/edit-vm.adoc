...
:description: Edit a virtual machine by clicking the ⋮ button and selecting Edit Config to change settings like CPU, memory, and network configurations, then apply
:revdate: 2026-06-03
...
For more information, see xref:virtual-machines/access-vm.adoc#_dynamic_ssh_key_injection_via_qemu_guest_agent[Dynamic SSH key injection via QEMU guest agent].

== Applying changes

After saving the new configuration, you have two options for applying the changes.

* *Save and Restart*: Applies the changes and restarts the virtual machine immediately.
* *Save*: Saves the new configuration without restarting. The changes remain pending and do not take effect until you manually restart the virtual machine.

image::vm/edit-vm-save-popup.png[Restart virtual machine to apply configuration changes]

![edit-vm-save-popup](/img/v1.8/vm/)

=== Pending changes notification

If you choose to delay the restart, a notification appears on the *Virtual Machines* screen to remind you about the pending configuration changes.

image::vm/edit-vm-list-restart-warning.png[Virtual machine pending restart notification]
