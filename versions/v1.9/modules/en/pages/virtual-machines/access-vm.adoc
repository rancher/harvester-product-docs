...
:description: Access the virtual machine using VNC, serial console, or SSH client and inject SSH keys via cloud-init or QEMU guest agent
:revdate: 2025-05-15
...
{harvester-product-name} provides two ways to inject SSH public keys into virtual machines. Generally, these methods fall into two categories: <<#_static_key_injection,static injection>>, which places keys in the cloud-init script when the virtual machine is first powered on; and <<#_dynamic_key_injection,dynamic injection>>, which allows keys or basic auth to be updated dynamically at runtime.

[#_static_key_injection]
