...
:description: Create and manage virtual private clouds (VPCs) with subnets, routing, NAT, and network policies in SUSE Virtualization with Kube-OVN
:revdate: 2026-06-05
...
image::networking/kubeovn-integration.svg[SUSE Virtualization and Kube-OVN integration]
...
Each created subnet has a setting called <<#_natoutgoing_setting,`natOutgoing`>>, which enables network address translation (NAT) for traffic leaving the subnet and going to destinations outside the VPC. This setting is disabled by default. To enable it, you must edit the subnet's YAML configuration and set the value to `natOutgoing: true`.

image::networking/customize_nat_outgoing.png[natOutgoing setting enabled]

By default, subnets in different VPCs are unable to communicate directly. To enable secure and controlled communication between them, you must establish a <<#_vpc_peering,VPC peering>> connection. Without it, subnet traffic in each VPC remains completely isolated.
...
+
.. On the *Subnet:Create* screen, configure the <<#_subnet_settings,settings>> that are relevant to your environment.
+
[NOTE]
====
...
+
.. (Optional) Use the `dhcpV4Options` field to specify additional DHCP settings such as the DNS server passed to the virtual machine workloads.
+
Example:
+
[,yaml]
...
----
+
image::networking/subnet_dhcp_options.png[DHCPv4 options]
+
.. Click *Create*.
...
image::networking/sdn-virtualization-mapping.svg[Mapping of VPCs and subnets to overlay networks and virtual machines]
...
. <<#_creating_a_vpc,Create two VPCs>> named `vpcpeer-1` and `vpcpeer-2`.
+
...
The {harvester-product-name} UI provides an interactive topology map of your VPC infrastructure, displaying the relationships between VPCs, subnets, overlay networks, and virtual machines.

To view the topology for a VPC, go to *Networks -> Virtual Private Cloud*, locate your target VPC, and click *Topology*.

image::networking/vpc-network-topology.png[VPC network topology]

=== Topology layout

The topology map uses a hierarchical layout (VPC -> subnet -> overlay network -> virtual machine) that allows you to audit network paths and manage visibility dynamically.

|===
| Component | Description

| *VPC*
| Virtual private cloud that contains the visualized components

| *Peered VPC*
| Other VPCs connected via peering (labeled with "Peering" badge)

| *Subnet*
| Subnet with CIDR, gateway, and provider

| *Overlay network*
| Overlay network mapped to subnet (displayed when a non-OVN provider is used)

| *Virtual machine*
| Virtual machine with network interfaces, IP addresses, and MAC addresses
|===
