...
. Click *Create*.
+
image::networking/hnc-1.png[Configuring host networks without node selectors]
...
+
image::networking/hnc-2.png[Configuring host networks with node selectors]
...
image::networking/hnc-3.png[Enabling a custom underlay interface on the host network]

When the *Underlay* option option is enabled, {harvester-product-name} orchestrates the following system modifications:
