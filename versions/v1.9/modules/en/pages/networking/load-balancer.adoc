= Load Balancer
:description: Create a Harvester load balancer to distribute traffic across virtual machines and Kubernetes clusters
:revdate: 2026-05-06
...
. Go to the *Health Check* tab to enable health check and specify the parameters, including the *Port*, *Success Threshold*, *Failure Threshold*, *Interval*, and *Timeout* if the backend service supports health check. Refer to <<#_health_checks>> for more details.
+
...
When you delete a guest cluster that has the Harvester Cloud Provider enabled, {harvester-product-name} automatically cleans up all associated `LoadBalancer` resources.
