= Installation Issues
:description: Troubleshoot failed installations by checking hardware requirements, modifying cluster tokens, and collecting logs
:revdate: 2026-05-05
:page-revdate: {revdate}

The following sections contain tips to troubleshoot or get assistance with failed installations.

[#_logging_into_installer]
...
In order to confirm the issue, connect to your agent node (i.e. with xref:troubleshooting/operating-system.adoc#_how_to_log_in_to_a_harvester_node[SSH]) and check the `rancherd` service log with the following command:
...
{harvester-product-name} exposes a `readyz` endpoint that can be used to assess the health and readiness of the control plane.

You can probe the endpoint using a client network tool such as `curl`.
...
Replace the `$TOKEN` and `$VIP` variables with the actual values.

* `$TOKEN`: Cluster token that you configured during xref:installation-setup/methods/iso-install.adoc#_installation_steps[installation]
* `$VIP`: Real cluster VIP (Use the value of `kube-vip.io/requestedIP` in the output of xref:installation-setup/management-address.adoc#_how_to_get_the_vip_mac_address[this command].)

[NOTE]
====
For clusters using self-signed TLS certificates, run `curl` with the `-k` flag.
====
...
+
Please follow the guide in <<#_logging_into_installer>> to log in. And run the command to generate a tarball that contains troubleshooting information:
+
[,sh]
----
  supportconfig -k -c
