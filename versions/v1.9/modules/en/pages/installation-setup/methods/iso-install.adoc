= ISO Installation
:description: Install SUSE Virtualization from the ISO image on a bare metal server by following the steps to set up a new cluster or join an existing cluster
:revdate: 2026-05-06
...
[IMPORTANT]
====
Support for legacy BIOS booting was deprecated in v1.7.0 and will be removed in a later release. Existing {harvester-product-name} clusters that use this boot mode will continue to function, but upgrading to later versions may require re-installation in UEFI mode. Starting with v1.8.0, the installer ISO does not boot on legacy BIOS systems to ensure that UEFI is used for all new deployments.
====
...
. Select the disks designated for cluster installation and virtual machine data storage.
+
...
. (Optional) You can import SSH keys by providing an HTTP URL (for example, `+https://github.com/<username>.keys+`). Once a key is configured, you can enable or disable password authentication for SSH access.
+
image::install/import-ssh-keys.png[HTTP URL for importing SSH keys]
+
image::install/enable-pass-access.png[Password authentication enabled]
