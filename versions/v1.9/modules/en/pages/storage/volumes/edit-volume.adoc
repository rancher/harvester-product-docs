= Edit a Volume
:description: Edit a volume to change its size, cancel a failed expansion, or change the StorageClass using a snapshot
:revdate: 2025-07-31
...
* *{longhorn-product-name}*: Both the V1 and V2 Data Engines support online volume expansion.
+
[IMPORTANT]
====
Before expanding a volume, check its status on the *Volumes* screen of the {harvester-product-name} UI and resolve any warnings, such as `Precheck new replica failed: disks are unavailable`. Volume expansion may fail if these warnings are not addressed.
====
