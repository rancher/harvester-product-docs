...
====
+
* `restoreVM`: Option that enables Harvester to automatically restore previously running xref:../virtual-machines/live-migration.adoc#_non_migratable_virtual_machines[non-migratable virtual machines] after the upgrade is *successfully* completed. You can specify either of the following values:
+
