...
When the nodes in the cluster do not use the HTTP Proxy to communicate with each other after the first node is installed successfully, you need to configure xref:/installation-setup/airgap.adoc#_configure_an_http_proxy_in_during_installation[http-proxy.noProxy] against the CIDR used by those nodes.
