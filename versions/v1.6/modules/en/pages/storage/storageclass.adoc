...
CAUTION: If a StorageClass with a replica count of `1` is used to create a volume that is attached to a virtual machine, that virtual machine is considered xref:../virtual-machines/live-migration.adoc#_non_migratable_virtual_machines[non-migratable].
