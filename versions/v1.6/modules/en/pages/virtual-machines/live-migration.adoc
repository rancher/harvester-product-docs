= Live Migration
:description: Move a virtual machine between hosts without downtime by ensuring the target node has matching resources, compatible CPU models, and meets scheduling rules
:revdate: 2026-01-27
...
[CAUTION]
====
Due to a https://kubevirt.io/user-guide/user_workloads/vm_rollout_strategies/#restartrequired-condition[limitation] in the VM rollout strategy implementation, virtual machines that are in the _pending restart_ state may be live-migrated to a random node instead of the specified target node. The virtual machines must be restarted for subsequent node-specific live migrations to work.
====
