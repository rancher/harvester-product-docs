...
* The CPU, memory, xref:./create-vm.adoc#_volumes[volumes], devices and other resources requested by the virtual machine can be copied or rebuilt on the migration target node while the source virtual machine is still running.
...
* xref:./create-vm.adoc#_node_scheduling[Node selector](./create-vm.md#node-scheduling) that binds the virtual machine to a specific node
