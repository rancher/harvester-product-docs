...
CAUTION: Exercise extreme caution when editing `/oem/90_custom.yaml`. Do not change the file's formatting and other settings to avoid breaking the system.

. Create a backup of the `/oem/90_custom.yaml` file on each node. You can use this backup to restore the file when the node is unable to reboot.

. Stop all virtual machines that are attached to the `mgmt` network.

. (Optional) Disable the xref:./storage-network.adoc#_storage_network_setting[storage network] if it uses `mgmt`.

. Change the MTU value in the `/oem/90_custom.yaml` file on each node. Locate the following paths, and then change the value in `MTU=1500`:
+
[bash, shell]
----
...
. Verify that the file's formatting is still valid using the `yq -e /oem/90_custom.yaml` command. This command prints the file's contents.
...
. (Optional) Enable the xref:./storage-network.adoc#_storage_network_setting[storage network] that you disabled before changing the MTU value.

. Start all virtual machines that are attached to `mgmt`.

. Verify that the virtual machine workloads are running normally.

For more information, see xref:#_change_the_mtu_of_a_network_configuration_with_no_attached_storage_network[Change the MTU of a Network Configuration with an Attached Storage Network](#change-the-mtu-of-a-network-configuration-with-an-attached-storage-network).
