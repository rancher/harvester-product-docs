...
:description: Learn how the Harvester network uses Linux devices and protocols to build traffic paths between VMs and external networks
:revdate: 2026-01-20
...
----
+
Because untagged VLAN handling is no longer applied, physical switches connected to {harvester-product-name} hosts must now be configured strictly as trunk ports. These ports must accept tagged traffic and send traffic tagged with the VLAN ID used by the VM network.
+
