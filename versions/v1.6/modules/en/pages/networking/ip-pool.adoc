= IP Pool
:description: Create and manage IP pools for SUSE Virtualization load balancers that have multiple ranges, allocation history, and scope settings
:revdate: 2026-06-16
...
* Create an IP pool <<Creating an IP pool using the Rancher UI,using the {rancher-short-name} UI>>, which allows you to specify the target project, namespace, and guest Kubernetes cluster for better resource management and isolation.
