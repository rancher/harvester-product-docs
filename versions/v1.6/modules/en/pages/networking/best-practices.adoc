...
. On the xref:/virtual-machines/access-vm.adoc#_access_with_the_suse_virtualization_ui[*Virtual Machines* screen of the Harvester UI], verify that the status of all VMs is either __Running__ or __Stopped__.
...
There are one or more xref:./cluster-network.adoc#_create_a_new_cluster_network[Network Config] under every xref:./cluster-network.adoc#_cluster_network[Cluster Network] on Harvester. Each `Network Config` is backed by a `VlanConfig` CRD object.
