...
* Check that your hardware meets the xref:../install/requirements.adoc#_hardware_requirements[minimum requirements] to complete installation.
...
For more information, see xref:../install/pxe-boot-install.adoc#_dhcp_server_configuration[DHCP Server Configuration].
...
In order to confirm the issue, connect to your agent node (i.e. with xref:./os.adoc#_how_to_log_in_to_a_harvester_node[SSH] and check the `rancherd` service log with the following command:
...
[NOTE]
====
A failure PXE Boot installation automatically generates a tarball if the xref:../install/harvester-configuration.adoc#_installdebug[`install.debug`] field is set to `true` in the Harvester configuration file.
====