...
Single-node clusters do not support xref:../virtual-machines/live-migration.adoc[Live Migration], so virtual machines become unavailable during cluster upgrades. Harvester forcibly shuts down all virtual machines before starting the upgrade process. You can configure Harvester to automatically restore running virtual machines using the `restoreVM` option of the xref:./config/settings.adoc#upgrade-config[`upgrade-config`] setting.
