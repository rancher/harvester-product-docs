...
`server_url` is the URL of the {product-name} cluster, which is used for the new `node` to join the cluster.
...
To ensure a high availability (HA) {product-name} cluster, please use either the cluster <<`install.vip`,VIP>> or a domain name in `server_url`.
====
...
Refer to the following example config for installing an RPM package in {product-name}:
...
To set up an HTTP(S) proxy for {product-name} components such as fetching external images and backup to S3 services,
...
Subsystem used to configure the OpenSSH Daemon (sshd). {product-name} currently only supports `sftp`.
...
*Definition*: Setting that defines the default add-on status. {product-name} add-ons are disabled by default.
...
*Definition*: Device on which the {product-name} operating system is installed.
...
{product-name} uses GPT partitioning on UEFI and BIOS systems by default. Compatibility issues may require you to use MBR partitioning instead.

If you specify the same storage device for both `install.device` and `install.data_disk`, {product-name} creates an additional partition for storing VM data. This additional partition is not created when you force usage of MBR partitioning. Instead, VM data is stored in a partition that stores OS data.
...
*Definition*: URL of ISO image to be downloaded and used to install {product-name} when booting from the kernel or vmlinuz.

=== `install.management_interface`

*Definition*: Network interfaces for the host machine.

{product-name} uses the https://www.freedesktop.org/software/systemd/man/systemd.net-naming-scheme.html[systemd net naming scheme]. Ensure that the interface name is present on the target machine before installation.
...
* `create`: Create a new {product-name} cluster.
* `join`: Join an existing {product-name} cluster. You must specify the `server_url`.
...
*Definition*: Setting that forces the installer to only install the {product-name} hypervisor (without any configuration). You must enable `harvester.install.automatic` to use this setting.
...
* `management`: Allows a node to be prioritized when {product-name} promotes nodes to management nodes.
...
*Definition*: VIP of the {product-name} management endpoint.
...
* `insecure`: When set to `true`, {product-name} does not verify the server's certificate. The default value is `false`.
...
The value of this field is typically derived from the kernel parameter `harvester.install.with_net_images`. When the value is `true`, {product-name} does not preload images packaged in the installation medium, and instead pulls images from the internet when necessary.
...
Overwriting system settings only works when {product-name} is installed in "create" mode.
If you install {product-name} in "join" mode, this setting is ignored.
