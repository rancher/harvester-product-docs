...
*Definition*: Additional trusted CA certificates that enable {product-name} to access external services.
...
*Definition*: Setting that allows {product-name} to automatically add disks that match the specified glob pattern as VM storage.
...
*Definition*: Configuration of a private registry created for the {product-name} cluster.
...
For security purposes, {product-name} automatically removes the username and password configured for the private registry after those credentials are stored in the `registries.yaml` file.
...
*Definition*: Number of seconds {product-name} waits before forcibly shutting down a VM that was stopped using the UI.

{product-name} sends a graceful shutdown signal to any VM that is stopped using the UI. If the graceful shutdown process is not completed within the specified number of seconds, {product-name} forcibly shuts down the VM.
...
{product-name} appends necessary addresses to user-specified `noProxy` values (for example,`localhost,127.0.0.1,0.0.0.0,10.0.0.0/8,longhorn-system,cattle-system,cattle-system.svc,harvester-system,.svc,.cluster.local`). This ensures that internal traffic flows as expected.
...
When set to `true`, {product-name} automatically loads the kernel modules required by the Longhorn V2 Data Engine, and attempts to allocate 1024 × 2 MiB-sized huge pages (for example, 2 GiB of RAM) on all nodes. 
...
A VM that is configured to use 2 CPUs (equivalent to 2,000 milliCPU) can consume the full allocation as long as the resources are available. However, if the host is running heavy workloads and an overcommit value is set (for example, 1600%), {product-name} only requests 125 milliCPU from the Kubernetes scheduler (2000/16 = 125 milliCPU).
...
{product-name} retrieves the ISO URL and checksum value from the `+${URL}/${VERSION}/version.yaml+` file that is accessible through the configured URL.
...
*Definition*: Version of {product-name} that is installed on nodes.
...
If you do not specify any values, {product-name} uses `TLSv1.2` and `ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305`.
...
*Default value*: `support-bundle-kit` image that is packed into the {product-name} ISO and is specific to each {product-name} release.
...
[NOTE]
====
* The value of `tag` in the `default` field is always based on the image that is packed into the {product-name} ISO. This field is automatically updated whenever the cluster is upgraded.
+
* The `default` field is used when the `value` field is not set or is left empty. {product-name} checks if the default image is stored in the cluster and is up-to-date.
+
...
+
* If your cluster is in an air-gapped environment and you specified a non-default image tag in the `value` field, ensure that the image is available in the local containerd registry. {product-name} is unable to xref:../../troubleshooting/operating-system.adoc#generate-a-support-bundle[generate a support bundle] if the image is not available.
====
...
*Definition*: Number of minutes {product-name} allows for the completion of the support bundle generation process.

The process is considered to have failed when the data collection and file packing tasks are not completed within the configured number of minutes. {product-name} does not continue or retry support bundle generation processes that have timed out. When the value is `0`, the timeout feature is disabled.
...
*Definition*: Number of minutes {product-name} waits before deleting a support bundle that has been packaged but not downloaded (either deliberately or unsuccessfully) or retained.

You can specify a value greater than or equal to 0. When the value is 0, {product-name} uses the default value.
...
*Definition*: Number of minutes {product-name} allows for collection of logs and configurations on the nodes for the support bundle.

If the collection process is not completed within the allotted time, {product-name} still allows you to download the support bundle (without the uncollected data). You can specify a value greater than or equal to 0. When the value is 0, {product-name} uses the default value.
...
+
The full ISO contains the core operating system components and all required container images. {product-name} can preload these container images to each node during installation and upgrades. When workloads are scheduled to management and worker nodes, the container images are ready to use.
+
...
[NOTE]
====
{product-name} deploys an upgrade-repo service on the cluster that serves as an HTTP server for nodes that need to preload the container images. When a `concurrency` value is set, each batch of nodes downloads the container images from this upgrade-repo in parallel. Because of this, you must consider the speed of the {product-name} management network and the read speed of the default disk for Longhorn.
====
+
* `restoreVM`: Option that enables {product-name} to automatically restore running virtual machines after a single-node cluster is upgraded. The default value is `false`, which causes all virtual machines to be stopped after the upgrade is completed. When set to `true`, {product-name} restarts virtual machines that were running before the upgrade was started. Virtual machines that were paused before the upgrade are not restarted.
...
*Definition*: JavaScript address for the plugin (when accessing {product-name} from Rancher).
