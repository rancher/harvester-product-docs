...
Then, the installer will apply the endpoint configuration and boot {product-name}. No further reboots will be required.

== Stream disk mode

SUSE has published a raw image artifact for pre-installed {product-name}. The installer now allows streaming a pre-installed image directly to disk to support better integration with cloud providers.
