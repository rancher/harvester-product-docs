...
+
* Earlier than v1.4.0: You must upload and configure the virtual machine images on the <<Upload the same VM images to a new cluster,new cluster>>. Ensure that the image names and configuration are identical so that the virtual machines can be restored.
