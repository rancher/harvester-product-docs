...
. Click `Add Rule` to specify the node affinity rules, e.g., for the xref:integrations/rancher/node-driver/node-driver.adoc#_topology_spread_constraints[topology spread constraints] use case, you can add the `region` and `zone` labels as follows:
+
[,yaml]
----
