...
One benefit of installing Kubernetes on node pools hosted by the Harvester Node Driver is that if a node loses connectivity with the cluster, {rancher-short-name} can automatically create another node to join the cluster to ensure that the count of the node pool is as expected.
