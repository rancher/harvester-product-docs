...
A VM network is an interface in a virtual machine that connects to the host network. As with a network configuration, every network except the built-in xref:networking/vm-network.adoc#_management_network[management network] must be under a cluster network.
...
Please refer to xref:networking/vm-network.adoc[network part] for more details about networks.
