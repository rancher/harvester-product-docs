...
. The *Management Network* is added by default with the following values:
 ...
 .. `Network`: The value `management Network` is set by default. You can't change this option if no other network has been created. See xref:../networking/vm-network.adoc[Harvester Network] for the full description on how to create new networks.
